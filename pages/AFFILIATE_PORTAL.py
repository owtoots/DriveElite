...
def generate_booking_receipt(ref_no, renter_name, vehicle, plate, travel_dates, amount, pickup_loc, return_loc):
    ...
    pdf.set_font("Helvetica", 'B', 16)
    pdf.cell(0, 10, "DRIVEELITE OFFICIAL BOOKING RECEIPT", ln=True, align='C')
    pdf.ln(5)
    
    pdf.set_font("Helvetica", '', 12)
    pdf.cell(0, 8, f"Receipt No: {ref_no}", ln=True)
    pdf.cell(0, 8, f"Date Issued: {datetime.date.today()}", ln=True)
    pdf.ln(8)
    
    pdf.set_font("Helvetica", 'B', 12)
    pdf.cell(0, 8, "CUSTOMER DETAILS:", ln=True)
    pdf.set_font("Helvetica", '', 11)
    pdf.cell(0, 8, f"Billed To: {renter_name}", ln=True)
    pdf.ln(5)
    
    pdf.set_font("Helvetica", 'B', 12)
    pdf.cell(0, 8, "RESERVATION DETAILS:", ln=True)
    pdf.set_font("Helvetica", '', 11)
    pdf.cell(0, 8, f"Vehicle: {vehicle} ({plate})", ln=True)
    pdf.cell(0, 8, f"Travel Dates: {travel_dates}", ln=True)
    pdf.cell(0, 8, f"Pickup: {pickup_loc}", ln=True)
    pdf.cell(0, 8, f"Return: {return_loc}", ln=True)
    pdf.ln(8)
    
    pdf.set_font("Helvetica", 'B', 12)
    pdf.cell(0, 8, "FINANCIAL SUMMARY:", ln=True)
    pdf.set_font("Helvetica", '', 12)
    pdf.cell(0, 8, f"Total Gross Rental Paid: Php {amount:,.2f}", ln=True)
    pdf.cell(0, 8, f"Payment Status: PAID / SECURED", ln=True)
    pdf.ln(25)
    
    pdf.set_font("Helvetica", 'I', 9)
    pdf.cell(0, 5, "This is a system-generated receipt and does not require a physical signature.", align='C', ln=True)
    pdf.cell(0, 5, "Thank you for choosing DriveElite!", align='C')
