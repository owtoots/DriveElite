...
import numpy as np
import time
import random # Required for generating the 6-digit reference numbers
            ...
            if f"contract_{t['id']}" in st.session_state:
                ...
                if can_proceed:
                    ...
                    if st.button("GENERATE CONTRACT & DISPATCH", key=f"ex_{t['id']}", type="primary", use_container_width=True):
                        ...
                        if not (chk_tank and chk_exterior and chk_deposit): st.error("Check off all items.")
                        elif len(bulk_photos) < 10: st.error("Upload 10 required.")
                            ...
                            try:
                                ...
                                if renter_email: send_pdf_email(renter_email, subject, body, pdf_bytes, f"Contract_{b_ref_display}.pdf")
                                if affiliate_email: send_pdf_email(affiliate_email, subject, "Partner Copy: " + body, pdf_bytes, f"Contract_{b_ref_display}.pdf")
                                st.rerun()
        ...
        with st.expander(f"🎫 #{b_ref} | RECEIVE RETURN: {t['make']} {t['model']} ({t['plate']}) - Renter: {t['renter_fullname']}"):
            if f"ret_receipt_{t['id']}" in st.session_state:
                ...
                if st.button("CLOSE BOOKING & RELEASE RENTER", key=f"fin_ret_{t['id']}", type="primary", use_container_width=True):
                    ...
                    # FIX: We removed 'amount = amount - ?'. Revenue (₱1,300) stays intact.
                    conn.execute("""
                        UPDATE bookings 
                ...
                with c1:
                    ...
                    if not d_ok:
                        ...
                        else: damage_deduct = st.number_input("Estimate", step=500.0, key=f"d_est_{t['id']}")
                    ...
                    if refund_amount < 0: st.error(f"OWES EXTRA: Php {abs(refund_amount):,.2f}"); refund_amount = 0.0
                    else: st.success(f"Deposit Refund: Php {refund_amount:,.2f}")
                with c2:
                    ...
                    with c_ret1:
                        ...
                        if st.button("Clear Renter", key=f"btn_sret_{t['id']}", use_container_width=True): st.session_state[f"clr_sret_{t['id']}"] += 1; st.rerun()
                    with c_ret2:
                        ...
                        if st.button("Clear Partner", key=f"btn_sreta_{t['id']}", use_container_width=True): st.session_state[f"clr_sreta_{t['id']}"] += 1; st.rerun()
                if st.button("GENERATE SETTLEMENT & COMPLETE JOURNEY", key=f"comp_{t['id']}", type="primary", use_container_width=True):
                    ...
                    if not d_ok and not img_damage: st.error("Damage photo required.")
                        ...
                        try:
                            ...
                            else: st.session_state[f"dmg_img_{t['id']}"] = None
                            emails_df = pd.read_sql_query("SELECT email FROM users WHERE username IN (?, ?)", conn, params=(t['renter_username'], st.session_state.username))
                            ...
                            if renter_email: send_pdf_email(renter_email, subject, body, pdf_bytes, f"Settlement_{b_ref_display}.pdf")
                            if affiliate_email: send_pdf_email(affiliate_email, subject, "Partner Copy: " + body, pdf_bytes, f"Settlement_{b_ref_display}.pdf")
                            st.rerun()
                        except Exception as e: st.error(str(e))
                    else: st.error("NOTICE: Both must sign.")
