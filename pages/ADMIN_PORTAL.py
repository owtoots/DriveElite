...
2. Alignment: How to call this in ADMIN_PORTAL.py
To ensure the Admin Portal displays these new settings correctly, go to the section where you manage pricing (likely your "Settings" or "Financials" tab) and call them like this:

Python
# Inside your Admin Portal Tab logic:
with st.expander("🛠️ Advanced Pricing & Margin Controls", expanded=True):
    render_platform_settings(conn)
    st.divider()
    render_admin_discount_table(conn)
3. Verification Steps
Unique Keys: I added key="fee_r" and key="share_a" to the number inputs to prevent Streamlit duplicate widget errors.

Modular Design: calculate_tiered_pricing is now a "pure" logic function—it doesn't print anything to the screen, making it safe to use in the Renter Portal for background math.

Seeding: The init_discount_db will only add the default values if the tables are completely empty, so it won't overwrite your custom settings every time the app restarts.

How does the alignment look on your Admin dashboard now?
