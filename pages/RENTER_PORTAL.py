...
if not st.session_state.get('logged_in') or st.session_state.get('role') != 'RENTER':
    ...
    with st.form("login_renter"):
        ...
        if st.form_submit_button("LOGIN TO SHOWROOM", use_container_width=True):
            # USING platform_users FOR V2 COMPATIBILITY
...
try:
    promo_q = "SELECT title, message FROM admin_promos WHERE active = 1 AND (target = 'RENTERS' OR target = 'ALL USERS') ORDER BY id DESC LIMIT 1"
    promo = pd.read_sql_query(promo_q, conn)
    if not promo.empty:
        st.info(f"📢 **{promo.iloc[0]['title']}**: {promo.iloc[0]['message']}")
except:
    pass
            ...
            if active_trips.empty:
                ...
                for _, t in active_trips.iterrows():
                    with st.expander(f"🚗 {t['make']} {t['model']} ({t['plate']}) | STATUS: {t['status']}"):
                        ...
                        st.write(f"**Pickup Loc:** {t['pickup_loc']}")
                        st.write(f"**Return Loc:** {t['return_loc']}")
                        ...
                        b_ref = t['booking_ref']
                        host_username = t['owner_username']
                        ...
                        with c_img:
                            r_img = st.file_uploader("📷", type=['jpg','png','jpeg'], key=f"r_img_act_{b_ref}", label_visibility="collapsed")
                        with c_msg:
                            r_input = st.text_input("Reply to host...", key=f"r_in_act_{b_ref}")

                        if st.button("Send Message", key=f"r_btn_act_{b_ref}", use_container_width=True):
                            if r_input or r_img:
                                path = save_chat_image(r_img, b_ref) if r_img else ""
                                text = r_input if r_input else "📸 Sent a photo."
                                conn.execute("INSERT INTO chat_messages (booking_ref, sender_username, receiver_username, message_text, image_path) VALUES (?, ?, ?, ?, ?)", (b_ref, st.session_state.username, host_username, text, path))
                                conn.commit()
                                st.rerun()

                        st.divider()
                        if t['status'] == 'PENDING': st.warning("Waiting for Affiliate to review your booking...")
                        elif t['status'] == 'CONFIRMED': st.success("Booking confirmed! Coordinate via chat.")
                        elif t['status'] == 'ONGOING': st.info("Trip is in progress. Drive safely!")
