                            ...
                            with st.container(border=True):
                                ...
                                with st.popover(f"⚡ BOOK {car['model'].upper()} NOW", use_container_width=True):
                                    ...
                                    if st.session_state[stage_key] == 0:
                                        ...
                                        for _, row in existing_bookings.iterrows():
                                            try:
                                                ...
                                                for i in range((e_dt - s_dt).days + 1): booked_dates.add((s_dt + datetime.timedelta(days=i)).strftime("%Y-%m-%d"))
                                            except: pass
                                    ...
                                    # ==========================================
                                       ...
                                       # Built line-by-line to prevent Streamlit from turning it into raw text
                                        cal_html = "<div style='background-color: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 12px; padding: 15px; margin-bottom: 20px;'>"
                                        cal_html += f"<h4 style='text-align: center; color: #0F172A; margin-top: 0px; margin-bottom: 10px; font-size: 16px;'>{month_name} {cal_year} Availability</h4>"
                                        cal_html += "<table style='width: 100%; border-collapse: separate; border-spacing: 4px; text-align: center; font-size: 13px;'>"
                                        cal_html += "<tr><th>Mo</th><th>Tu</th><th>We</th><th>Th</th><th>Fr</th><th>Sa</th><th>Su</th></tr>"
